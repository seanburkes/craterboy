...
mod tests {
    ...
    #[test]
    fn cpu_cb_cycles_for_res_set_hl_vs_reg() {
        let mut rom = vec![0; ROM_BANK_SIZE];
        rom[0x0000] = 0x06;
        rom[0x0001] = 0x01;
        rom[0x0002] = 0x21;
        ...
        let cycles = cpu.step(&mut bus).expect("cb res 0,b");
        assert_eq!(cycles, 8);
        let cycles = cpu.step(&mut bus).expect("cb res 0,(hl)");
        assert_eq!(cycles, 16);

        let cycles = cpu.step(&mut bus).expect("cb set 0,b");
        assert_eq!(cycles, 8);
